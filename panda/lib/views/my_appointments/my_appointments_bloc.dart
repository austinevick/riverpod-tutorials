...
class MyAppointmentBloc extends Bloc<MyAppointmentEvent, MyAppointmentState> {
  MyAppointmentBloc(super.initialState);

  @override
  void onTransition(
      Transition<MyAppointmentEvent, MyAppointmentState> transition) {
    print(transition);
  ...
  Stream<MyAppointmentState> mapEventToState(MyAppointmentEvent event) async* {
    ...
    if (event is InitMyAppointment) {
      ...
      List<AppointmentRow> vitalList = [];
      var response = await repository.getAppointments(id!);
      if (response.status == 200) {
        response.data!.appointments!.forEach((v) {
          vitalList.add(AppointmentRow(data: v));
        });
