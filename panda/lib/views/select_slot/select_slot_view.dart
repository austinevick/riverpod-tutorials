...
class SelectSlotView extends StatelessWidget {
  ...
  SelectSlotView({Key? key, required this.data}) : super(key: key);

  late String initialDate;
  late SelectSlotBloc _bloc;
                    ...
                    children: <Widget>[
                      Card(
                          ...
                          child: Column(children: [
                            ...
                            Container(
                                height: 1, color: Colors.grey[300], width: 400),
                            const SizedBox(height: 10),
                            ListTile(
                              title: Center(
                                child: CustomText(
                                    text: '           Today, $initialDate',
                                    color: Colors.black,
                                    small: true,
                                    bold: true),
                              ),
                            ...
                            BlocBuilder<SelectSlotBloc, SelectSlotState>(
                                buildWhen: (previous, current) =>
                                    previous.nextAvailableSlot !=
                                    current.nextAvailableSlot,
                                builder: (context, state) {
                                  if (state.nextAvailableSlot == null) {
                                    return CustomButton(
                                      label: 'loading next day availability',
                                      key: const Key('nextDay1'),
                                    );
                                  } else {
                                    return CustomButton(
                                        label:
                                            'Next day availability on ${state.nextAvailableSlot}',
                                        key: const Key('nextDay'),
                                        onPress: () => _bloc.add(InitBlock(
                                            data,
                                            context,
                                            _refreshController,
                                            state.nextAvailableSlot!)));
                                  }
                                }),
                            const SizedBox(height: 20),
                            const CustomText(
                            ...
                            FloatingActionButton(
                                onPressed: () {
                                  Navigator.pushNamed(
                                      context, ConfirmBookingViewRoute);
                                },
