...
class MyVitalsBloc extends Bloc<MyVitalsEvent, MyVitalsState> {
  MyVitalsBloc(super.initialState);
  ...
  Stream<MyVitalsState> mapEventToState(MyVitalsEvent event) async* {
    ...
    if (event is InitMyVitals) {
      ...
      List<VitalRow> vitalList = [];
      var response = await repository.getVitals();
      if (response.status == 200) {
        response.data!.forEach((v) {
          vitalList.add(VitalRow(data: v));
        });
      ...
      var response = await repository.vitals();
      print('option: ${response.data}');
      yield state.copyWith(vitalOptions: response.data);
    }
  }
